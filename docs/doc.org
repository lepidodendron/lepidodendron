...
alternatively we simply take the predicted glyphs and match each to the character with the minimal mse,
and use those characters for computing the bleu scores.
...
think of glyphs as scales (leaf wounds)
