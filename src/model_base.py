...
def model(mode
    ...
    with scope('target'):
        ...
        # time major order
        tgt_idx = tf.transpose(tgt_idx, (1,0)) # t n

        # causal padding
        fire = self.fire = tf.pad(tgt_idx, ((1,0),(0,0)), constant_values= 2)
...
if '__main__' == __name__:
    ...
    src_idx, len_src, tgt_idx, len_tgt = pipe(batch, (tf.int32, tf.int32, tf.int32, tf.int32))
    train = model('train', cws.dwh(), cwt.dwh(), src_idx, len_src, tgt_idx, len_tgt)
    ...
    def log(step
            ...
            , log= tf.summary.merge(
                (  tf.summary.scalar('step_xid', dummy[0])
